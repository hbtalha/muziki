...
class AudioPlayer {
    ...
    if (!_handleAudioSessionActivation || await audioSession.setActive(true)) {
      ...
      if (requireActive) {
        if (_active) {
          ...
          _setPlatformActive(true, playCompleter: playCompleter)?.catchError((dynamic e) {});
        }
      }
    } else {
  ...
  Future<void> moveQueueItem(int currentIndex, int newIndex) async {
    ...
    _audioSource!._move(currentIndex, newIndex);
    _updateShuffleIndices();
    await (await _platform).setShuffleOrder(SetShuffleOrderRequest(audioSourceMessage: _audioSource!._toMessage()));
  }

  /// Sorts the indices of the shuffle order of the currently loaded [AudioSource] according to [PlaylistSorting] and [sortingOrder].
  Future<void> sort(List<Song> songs, PlaylistSorting playlistSorting, {SortingOrder sortingOrder = SortingOrder.ascending}) async {
    if (_disposed) return;
    if (audioSource == null) return;
    _audioSource!._sort(songs, playlistSorting, sortingOrder: sortingOrder, initialIndex: currentIndex);
    _updateShuffleIndices();
    await (await _platform).setShuffleOrder(SetShuffleOrderRequest(audioSourceMessage: _audioSource!._toMessage()));
  }
...
abstract class AudioSource {
  ...
  void _sort(List<Song> songs, PlaylistSorting playlistSorting, {SortingOrder sortingOrder = SortingOrder.ascending, int? initialIndex});
  void _move(int currentIndex, int newIndex);
...
class ConcatenatingAudioSource extends AudioSource {
  ...
  @override
  void _sort(List<Song> songs, PlaylistSorting playlistSorting, {SortingOrder sortingOrder = SortingOrder.ascending, int? initialIndex}) {
    _shuffleOrder.sort(songs, playlistSorting, sortingOrder: sortingOrder);
  }

  @override
  void _move(int currentIndex, int newIndex) {
    _shuffleOrder.move(currentIndex, newIndex);
  }
...
abstract class ShuffleOrder {
  ...
  void sort(List<Song> songs, PlaylistSorting playlistSorting, {SortingOrder sortingOrder = SortingOrder.ascending, int? initialIndex});
...
class DefaultShuffleOrder extends ShuffleOrder {
  ...
  @override
  Future<void> sort(List<Song> songs, PlaylistSorting playlistSorting,
      {SortingOrder sortingOrder = SortingOrder.ascending, int? initialIndex}) async {
    assert(initialIndex == null || indices.contains(initialIndex));
    if (indices.length <= 1) return;
    if (initialIndex == null) return;

    List<Song> songsSorted = sortedSongs(songs, playlistSorting, sortingOrder: sortingOrder);

    indices.clear();
    for (var s in songsSorted) {
      indices.add(songs.indexOf(s));
    }
  ...
  @override
  void move(int currentIndex, int newIndex) {
    indices.insert(newIndex, indices.removeAt(currentIndex));
  }
